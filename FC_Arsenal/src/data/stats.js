export const playerStats = {
    1: {
        // Bukayo Saka
        matches: 38,
        goals: 15,
        assists: 12,
        minutesPlayed: 3200,
        bio: "Bukayo Saka is Arsenal's star boy, known for his incredible work rate, skill, and ability to deliver in clutch moments.",
        image: "https://resources.premierleague.com/premierleague/photos/players/250x250/p223340.png",
    },
    2: {
        // Martin Ødegaard
        matches: 36,
        goals: 12,
        assists: 10,
        minutesPlayed: 3100,
        bio: "The captain leads by example with his pressing and creative vision, unlocking defenses with surgical precision.",
        image: "https://resources.premierleague.com/premierleague/photos/players/250x250/p184029.png",
    },
    3: {
        // Declan Rice
        matches: 37,
        goals: 5,
        assists: 8,
        minutesPlayed: 3300,
        bio: "A record signing who has instantly become a fan favorite, dominating the midfield with his power and reading of the game.",
        image: "https://resources.premierleague.com/premierleague/photos/players/250x250/p204480.png",
    },
    4: {
        // Gabriel Jesus
        matches: 25,
        goals: 8,
        assists: 5,
        minutesPlayed: 1800,
        bio: "Gabriel Jesus brings relentless energy and elite pressing to the front line, alongside his clinical finishing.",
        image: "https://resources.premierleague.com/premierleague/photos/players/250x250/p205651.png",
    },
    5: {
        // William Saliba
        matches: 38,
        goals: 2,
        assists: 1,
        minutesPlayed: 3420,
        bio: "Cool, calm, and collected. Saliba has established himself as one of the best center-backs in the league.",
        image: "https://resources.premierleague.com/premierleague/photos/players/250x250/p462424.png",
    },
    6: {
        // Ben White
        matches: 35,
        goals: 4,
        assists: 4,
        minutesPlayed: 3000,
        bio: "Versatile and technically gifted, Ben White formed a crucial part of Arsenal's solid defense, often joining the attack.",
        image: "https://resources.premierleague.com/premierleague/photos/players/250x250/p225321.png",
    },
    7: {
        // Gabriel Magalhães
        matches: 36,
        goals: 5,
        assists: 0,
        minutesPlayed: 3150,
        bio: "A warrior at the back, Gabriel combines physical dominance with set-piece threat.",
        image: "https://resources.premierleague.com/premierleague/photos/players/250x250/p226597.png",
    },

    9: {
        // Gabriel Martinelli
        matches: 35,
        goals: 15,
        assists: 6,
        minutesPlayed: 2800,
        bio: "Electric pace and boundless energy define Martinelli's game. A nightmare for defenders down the left flank.",
        image: "https://resources.premierleague.com/premierleague/photos/players/250x250/p444145.png",
    },
    10: {
        // Kai Havertz
        matches: 37,
        goals: 13,
        assists: 7,
        minutesPlayed: 2900,
        bio: "Growing into his role, Havertz offers height, technique, and late runs into the box that make him a unique weapon.",
        image: "https://resources.premierleague.com/premierleague/photos/players/250x250/p219847.png",
    },
    11: {
        // Leandro Trossard
        matches: 32,
        goals: 10,
        assists: 3,
        minutesPlayed: 1500,
        bio: "The definition of an impact player. Trossard can start or come off the bench to score crucial goals.",
        image: "https://resources.premierleague.com/premierleague/photos/players/250x250/p116216.png",
    },

    13: {
        // Jorginho
        matches: 20,
        goals: 0,
        assists: 2,
        minutesPlayed: 1100,
        bio: "Experience and composure personified. Jorginho dictates play and organizes the team around him.",
        image: "https://resources.premierleague.com/premierleague/photos/players/250x250/p85955.png",
    },
    14: {
        // Takehiro Tomiyasu
        matches: 21,
        goals: 1,
        assists: 1,
        minutesPlayed: 1200,
        bio: "A defensive specialist who can play anywhere across the backline with equal proficiency.",
        image: "https://resources.premierleague.com/premierleague/photos/players/250x250/p223723.png",
    },
    18: {
        // Riccardo Calafiori
        matches: 30, // Mock stats
        goals: 2,
        assists: 5,
        minutesPlayed: 2500,
        bio: "A modern, versatile defender with exceptional ball-playing ability and composure, Calafiori is a rising star in European football.",
        image: "https://img.uefa.com/imgml/TP/players/3/2024/cutoff/250113392.png",
        // Placeholder or find a better one later
    },
    16: {
        // Eddie Nketiah
        matches: 24,
        goals: 5,
        assists: 2,
        minutesPlayed: 1000,
        bio: "A poacher with sharp movement in the box, Nketiah is always ready to capitalize on a chance.",
        image: "https://resources.premierleague.com/premierleague/photos/players/250x250/p205533.png",
    },
    17: {
        // David Raya
        matches: 32,
        goals: 0,
        assists: 0,
        minutesPlayed: 2880,
        bio: "Golden Glove winner with exceptional distribution, Raya has been integral to Arsenal's build-up play.",
        image: "https://resources.premierleague.com/premierleague/photos/players/250x250/p154566.png",
    },
};
